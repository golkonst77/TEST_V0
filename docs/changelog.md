...
## [2024-12-19] - Настройка WhatsApp API и исправление квиза

### Добавлено
- Интеграция с сервисом WhatsApp API `whapi.cloud`
- Автоматическая отправка сообщений через квиз
- Fallback механизм - открытие ссылки `wa.me` при ошибках API
- Обновленный текст в квизе: "ЗВОНИТЬ НЕ БУДЕМ, ТОЛЬКО СООБЩЕНИЕ!"

### Изменено
- Обновлен токен WhatsApp API на `K9edm63ZcOVma3QQQZy4vQM7JQOSI1RF`
- Улучшена обработка ошибок в функциях отправки WhatsApp
- Увеличен размер кнопок "Дополнительные услуги" с 100px до 120px

### Исправлено
- Проблемы с прозрачностью в dropdown компонентах
- Фон калькулятора на главной странице
- Центрирование текста в секции контактов

### Статус WhatsApp API
- ✅ **Текстовые сообщения**: Работают корректно
- ❌ **Отправка файлов**: Не работает (требует диагностики)
- 🔧 **Сервис**: whapi.cloud
- 🔑 **Токен**: K9edm63ZcOVma3QQQZy4vQM7JQOSI1RF
- 📱 **Номер отправителя**: 79533301777 (WhatsApp Business)
