...
## [✅ Версия обновлена: 1.1.2 → 1.1.2
1.1.2] - 2025-10-07 - Автоматическое обновление версии

### Изменено
- Автоматическое обновление версии до ✅ Версия обновлена: 1.1.2 → 1.1.2
1.1.2
- Обновлен package.json

## [✅ Версия обновлена: 1.1.1 → 1.1.1
1.1.1] - 2025-10-07 - Автоматическое обновление версии

### Изменено
- Автоматическое обновление версии до ✅ Версия обновлена: 1.1.1 → 1.1.1
1.1.1
- Обновлен package.json

## [1.1.0] - 2025-01-27 - Исправления API и новая страница переменных окружения

### Исправлено
- **Обработка ошибок в API endpoints**:
  - Исправлена ошибка 405 Method Not Allowed в `/api/admin/settings`
  - Добавлена поддержка POST метода как альтернативы PUT
  - Улучшена обработка JSON parsing ошибок в клиентском коде
  - Добавлена проверка Content-Type перед парсингом ответов сервера
- **Конфигурация Supabase**:
  - Исправлен порядок приоритета переменных окружения в `settings-store.ts`
  - Добавлен `NEXT_PUBLIC_SUPABASE_ANON_KEY` как приоритетный ключ
- **Обработка ошибок в админ-панели**:
  - Улучшена обработка ошибок в `admin/settings/page.tsx`
  - Добавлены уведомления через toast в `admin/header/page.tsx`
  - Более информативные сообщения об ошибках

### Добавлено
- **Страница переменных окружения**:
  - Новая страница `/admin/environment` для просмотра конфигурации
  - API endpoint `/api/admin/environment` для получения информации о среде
  - Возможность показать/скрыть секретные ключи
  - Кнопки для копирования значений в буфер обмена
  - Статус-индикаторы для каждой переменной (настроено/не настроено)
  - Предупреждение о безопасности при показе секретов
  - Интеграция в главное меню админ-панели
- **Тестовая страница API**:
  - Страница `/test-api` для диагностики API endpoints
  - Возможность тестирования различных HTTP методов
  - Детальная информация об ответах сервера

### Технические улучшения
- Улучшена обработка ошибок во всех API endpoints
- Добавлена более детальная диагностика проблем с конфигурацией
- Улучшена безопасность при работе с секретными ключами
...
### Добавлено
- Исправлена ошибка сохранения контактной информации в админ панели
- Улучшены скрипты деплоя с детальным выводом на английском языке
- Улучшено отображение версии в футере (убрана версия из хедера)
- **Добавлена полная документация по деплою** в `docs/DEPLOYMENT.md`
- **Обновлен README.md** с информацией о портах и скриптах деплоя

### Изменено
- `handleSave` в админ панели теперь отправляет один PUT запрос вместо двух
- API endpoint `/api/admin/settings` теперь обрабатывает header данные
- Компонент `VersionInfo` теперь сначала пытается получить версию через API
- Компонент `VersionBadge` удален из хедера
- **Документирована схема портов:** hvastik-alert.ru → 3001, prostoburo.com → 3000

### Исправлено
- Исправлен путь к `version.json` в API endpoint `/api/version`
- Исправлена синтаксическая ошибка в `lib/settings-store.ts`
- Улучшена обработка ошибок в скриптах деплоя
- **Исправлена проблема с портами** - теперь каждый сайт работает на своем порту
