  ...
  return (
    <Link href="/" className={`flex items-center space-x-2 ${className}`}>
      {logoConfig.type === "image" && logoConfig.imageUrl ? (
        ...
        <>
          <div className="h-12 w-12 flex-shrink-0 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 flex items-center justify-center">
            <span className="text-white font-bold text-lg">ПБ</span>
          </div>
          <span className="font-bold text-xl whitespace-nowrap">{logoConfig.text || siteName}</span>
        </>
      )}
    </Link>
  )
